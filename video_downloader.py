...
class video_downloader():
    def __init__(self, url):
        ...
        self.headers = {
            'Referer':
            'http://api.xfsub.com/xfsub_api/?url=%s?qqdrsign=055a4' % self.url
        }
    ...
    def get_key(self):
        ...
        req.encoding = 'utf-8'
        rst = re.findall('"url.php",\ (.+),', req.text)
        self.info = {'time':''}
        if len(rst) > 0:
            self.info = json.loads(rst[0])  #使用正则表达式匹配结果，将匹配的结果存入info变量中
    ...
    def get_url(self):
        ...
        req = self.s.post(
            url=self.get_url_api, data=data, headers=self.headers)
    ...
    def video_download(self, url, filename):
        request.urlretrieve(
            url=url, filename=filename, reporthook=self.Schedule)
